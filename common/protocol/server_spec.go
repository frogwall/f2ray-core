...
type ServerSpec struct {
	...
	method string // for brook protocol
}
...
func NewServerSpecWithMethod(dest net.Destination, valid ValidationStrategy, method string, users ...*MemoryUser) *ServerSpec {
	return &ServerSpec{
		dest:   dest,
		users:  users,
		valid:  valid,
...
func NewServerSpecFromPB(spec *ServerEndpoint) (*ServerSpec, error) {
	...
	return NewServerSpecWithMethod(dest, AlwaysValid(), method, mUsers...), nil
}
...
func (s *ServerSpec) Method() string {
	s.RLock()
	defer s.RUnlock()
	return s.method
}
